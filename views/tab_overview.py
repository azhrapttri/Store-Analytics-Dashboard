"""
Tab 1 — Overview
Displays KPI cards, store comparison charts, revenue/rental trends,
customer segmentation, and a top-category summary.
"""
...
def render(kpi, store_df, trend_df, trend_type, cust_seg_df, cat_df, month_f, palette, chart_theme):
    ...
    if not kpi.empty:
        ...
        for col, (icon, lbl, val, sub) in zip(st.columns(6), kpi_items):
            col.markdown(f"""
            <div class="kpi-card">
    ...
    # May 2005 data-artifact warning
    if month_f == "2005-05":
        st.markdown(f"""
    ...
    with c1:
        ...
        st.plotly_chart(chart_theme(fig, h=190, t=8), use_container_width=True)
    ...
    # ── Row 2: Customer pie + rental trend + top categories ──────────────────
    ...
    with c4:
        ...
        if not cust_seg_df.empty:
            ...
            st.plotly_chart(chart_theme(fig, h=180, t=8), use_container_width=True)
    ...
    if not cat_df.empty:
        ...
        store_insight = _build_store_insight(store_df, P)
...
def _build_store_insight(store_df, palette) -> str:
    """Return a plain-text sentence comparing the two stores."""
    if store_df.empty or len(store_df) < 2:
        return "Store filter active — showing single store data."

    s1_rev  = store_df[store_df["store_id"] == "Store 1"]["revenue"].values
    s2_rev  = store_df[store_df["store_id"] == "Store 2"]["revenue"].values
    s1_rent = store_df[store_df["store_id"] == "Store 1"]["rentals"].values
    s2_rent = store_df[store_df["store_id"] == "Store 2"]["rentals"].values

    if not (len(s1_rev) and len(s2_rev)):
        return "Store comparison reflects actual transaction location."

    leading       = "Store 1" if float(s1_rev[0]) >= float(s2_rev[0]) else "Store 2"
    leading_rev   = max(float(s1_rev[0]), float(s2_rev[0]))
    leading_rent  = int(s1_rent[0]) if leading == "Store 1" else int(s2_rent[0])
    return (f"<strong>{leading}</strong> leads with "
