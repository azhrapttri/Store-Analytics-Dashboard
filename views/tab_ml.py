...
Revenue forecast (3-month outlook) and a business decision simulator.
Loads a pre-trained model artifact from revenue_forecast_model.pkl.
"""
...
MODEL_PATH = "revenue_forecast_model.pkl"


def render(palette, chart_theme):
    ...
    <div style="background:{P['surface']};border:1px solid {P['border']};border-radius:14px;
      ...
      <div style="font-size:0.78rem;color:{P['muted']};line-height:1.7;">
        This tool helps you <strong>predict next 3 months revenue</strong> and
        <strong>test business scenarios</strong> before making decisions.
        The forecast is based on your store's historical data (May 2005 – Feb 2006).
      </div>
    </div>
    ...
    fut_preds   = _generate_forecast(model, scaler, feature_cols, last_row, last_num, fut_dates, rev_ml)
    mae         = metrics["loo_mae"]
    ...
    fig = _build_forecast_chart(hist_x, rev_ml, fut_x_str, fut_preds, last_actual, mae, P)
    st.plotly_chart(chart_theme(fig, h=300, t=20), use_container_width=True)

    # ── Forecast cards ────────────────────────────────────────────────────────
    st.markdown(f"""
    <div style="font-size:0.7rem;color:{P['muted']};margin-bottom:8px;">
      <strong>What to expect:</strong> These are your projected revenue numbers.
      The range shows best/worst case based on historical patterns.
    </div>
    """, unsafe_allow_html=True)

    _render_forecast_cards(fut_labels, fut_preds, last_actual, mae, P)

    st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)

    # ── Business simulator ────────────────────────────────────────────────────
    _render_business_simulator(fut_preds, rev_ml, P)
...
def _generate_forecast(model, scaler, feature_cols, last_row, last_num, fut_dates, rev_ml) -> np.ndarray:
    """Iteratively predict revenue for the next 3 months."""
    fut_preds   = []
    ...
    for i in range(1, 4):
        ...
        current_row["transactions"]       = current_row["transactions"] * growth_factor
        current_row["unique_customers"]   = current_row["unique_customers"] * growth_factor
        current_row["unique_films_rented"] = current_row["unique_films_rented"] * growth_factor
        ...
        if i == 1:
            ...
            current_row["transactions_lag1"] = current_row["transactions"] / growth_factor
            current_row["customers_lag1"]    = current_row["unique_customers"] / growth_factor

        recent_revs = list(rev_ml["revenue"].tail(1).values) + [current_row["revenue_lag1"]]
        ...
        X_fut        = np.array([[current_row.get(c, 0) for c in feature_cols]])
        X_fut_scaled = scaler.transform(X_fut)
        pred         = max(0, model.predict(X_fut_scaled)[0])
        fut_preds.append(pred)
        current_row["revenue"] = pred

    return np.array(fut_preds)


def _build_forecast_chart(hist_x, rev_ml, fut_x_str, fut_preds, last_actual, mae, P) -> go.Figure:
    """Build the combined historical + forecast Plotly figure."""
    ...
    # Zero-revenue markers (May 2005 artifact)
    ...
    fig.update_layout(
        ...
        hovermode="x unified",
        xaxis=dict(showgrid=True, tickfont=dict(size=9)),
        yaxis=dict(showgrid=True, tickfont=dict(size=9),
                   tickprefix="$", tickformat=",.0f"),
    )
    return fig


def _render_forecast_cards(fut_labels, fut_preds, last_actual, mae, P):
    """Render the three forecast summary cards side by side."""
    cols = st.columns(3)
...
def _render_business_simulator(fut_preds, rev_ml, P):
    """Render the what-if business decision simulator form and results."""
    ...
    <div style="background:{P['surface']};border:1px solid {P['border']};border-radius:14px;
      ...
      <div style="font-size:0.78rem;color:{P['muted']};line-height:1.7;">
        <strong>Test your ideas before spending money.</strong> Adjust the inputs below
        to simulate different business scenarios and see how they affect next month's revenue.
        Current baseline: <strong style="color:{P['text']};">${baseline:,.0f}</strong>.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.form(key="whatif_form", border=True):
        ...
        with wif1:
            price_adj = st.number_input(
                "Rental Price Change (%)", min_value=-20, max_value=50, value=0, step=5,
                help="Increase or decrease your average rental price. +10% means raising prices by 10%.",
            )
        with wif2:
            promo_pct = st.number_input(
                "Discount/Promo (%)", min_value=0, max_value=50, value=0, step=5,
                help="How much discount are you giving? 20% off = less revenue per rental but maybe more customers.",
            )
        with wif3:
            new_custs = st.number_input(
                "New Customers Target", min_value=0, max_value=500, value=0, step=10,
                help="How many NEW customers do you plan to attract? Each brings ~1.5 rentals.",
            )

        wif4, wif5 = st.columns(2)
        with wif4:
            inventory_add = st.number_input(
                "New Movie Titles", min_value=0, max_value=100, value=0, step=5,
                help="New films attract more rentals. Each title adds ~$15/month revenue.",
            )
        with wif5:
            staff_add = st.number_input(
                "Extra Staff Hired", min_value=0, max_value=10, value=0, step=1,
    ...
    if submitted:
        _render_simulation_results(
            baseline, avg_rpt, price_adj, promo_pct, new_custs, inventory_add, staff_add, P
        )
    else:
        _render_simulator_placeholder(P)


def _render_simulation_results(baseline, avg_rpt, price_adj, promo_pct, new_custs,
                                inventory_add, staff_add, P):
    """Display the simulation outcome after form submission."""
    ...
    total_delta_pct = (total_delta / baseline * 100) if baseline else 0
    ...
    <div style="font-size:0.78rem;font-weight:700;color:{P['accent_text']};
                text-transform:uppercase;letter-spacing:0.06em;margin:16px 0 12px;">
      Your Simulation Result
    </div>""", unsafe_allow_html=True)

    r1, r2 = st.columns([2, 3])
    with r1:
        price_dot = "🟢" if price_impact >= 0 else "🔴"
        inv_dot   = "🟢" if inventory_impact >= 0 else "⚪"
        staff_dot = "🟢" if staff_impact >= 0 else "⚪"
        st.markdown(f"""
        <div class="pred-card" style="margin-top:4px;">
          ...
          <div style="font-size:0.9rem;font-weight:700;color:{sim_col};margin:6px 0;">
            {sim_sym} {abs(total_delta_pct):.1f}% from baseline
          </div>
          <div style="font-size:0.68rem;color:{P['muted']};line-height:1.9;text-align:left;padding:4px 2px;">
            {price_dot} Price change: <strong>${price_impact:+,.0f}</strong><br>
            🔴 Discount cost: <strong>${promo_impact:,.0f}</strong><br>
            🟢 New customers: <strong>{new_custs} people (+${newcust_impact:,.0f})</strong><br>
            {inv_dot} New titles: <strong>{inventory_add} titles (+${inventory_impact:,.0f})</strong><br>
            {staff_dot} Extra staff: <strong>{staff_add} people (+${staff_impact:,.0f})</strong>
          </div>
        </div>""", unsafe_allow_html=True)

    with r2:
        msg_icon, msg_title, msg = _build_recommendation(
            total_delta, total_delta_pct, baseline, promo_pct, price_adj, new_custs
        )
        st.markdown(f"""
        <div style="background:{P['surface']};border:1px solid {P['border']};border-radius:14px;
...
def _build_recommendation(total_delta, total_delta_pct, baseline, promo_pct, price_adj, new_custs):
    """Return (icon, title, message) for the simulation recommendation panel."""
    if abs(total_delta_pct) < 2:
        return (
            "⚖️", "Minimal Change Expected",
            f"Your plan keeps revenue around <strong>${baseline:,.0f}</strong>. "
            "If you want bigger growth, try combining price increase + new titles + more customers.",
        )
    if total_delta > 0 and total_delta_pct > 15:
        return (
            "🚀", "Strong Growth Opportunity!",
            f"This plan could grow revenue by <strong>${total_delta:,.0f}</strong>! "
            f"Make sure you have enough staff and inventory to handle the extra demand. "
            f"Do you have {new_custs} new customers' worth of popular movies ready?",
        )
    if total_delta > 0:
        extra = (
            f"Watch your margins — the {promo_pct}% discount costs ${abs(total_delta * promo_pct / 100):,.0f}. "
            "Make sure you're not giving away too much."
            if promo_pct > 10
            else "Good balance of growth and profitability."
        )
        return ("✅", "Solid Improvement",
                f"Revenue should increase by <strong>${total_delta:,.0f}</strong>. {extra}")

    # Revenue will drop
    if promo_pct > 15:
        reason = (f"The {promo_pct}% discount is too deep without enough new customers. "
                  "Try reducing discount to 10% or attracting more customers.")
    elif price_adj < 0:
        reason = "Review your pricing — are you undercharging?"
    else:
        reason = "Your costs outweigh gains. Focus on customer acquisition first."
    return ("⚠️", "Revenue Will Drop",
            f"This plan loses <strong>${abs(total_delta):,.0f}</strong>. {reason}")


def _render_simulator_placeholder(P):
    """Show a prompt encouraging the user to fill in the simulator form."""
    st.markdown(f"""
