...
Generates the full <style> block injected into the Streamlit app,
parameterised by the active theme palette.
"""


def get_stylesheet(palette: dict) -> str:
    """Return a complete CSS string for the given palette."""
    P = palette
    return f"""
<style>
...
div[data-testid="metric-container"] label {{
    color: {P["muted"]} !important; font-size: 0.68rem !important; font-weight: 700 !important;
...
.stSelectbox div[data-baseweb="select"] > div {{
    ...
    border-radius: 10px !important; color: {P["text"]} !important;
}}
.stRadio label {{ font-size: 0.78rem !important; color: {P["text"]} !important; }}
div[data-testid="stToggle"] label {{
    color: {P["muted"]} !important; font-size: 0.72rem !important;
...
div[data-testid="stDataFrame"] [role="columnheader"] {{
    background: linear-gradient(135deg, {P["surface_2"]}, {P["surface"]}) !important;
    color: {P["pink_hot"]} !important; font-weight: 700 !important; font-size: 0.7rem !important;
    text-transform: uppercase !important; letter-spacing: 0.06em !important;
    border-bottom: 1px solid {P["border"]} !important;
}}
div[data-testid="stDataFrame"] [role="gridcell"] {{
    color: {P["text"]} !important; border-bottom: 1px solid {P["border"]} !important;
...
.data-note {{
    ...
    border: 1px solid rgba(236,72,153,0.30); border-radius: 10px; padding: 8px 14px;
    font-size: 0.68rem; color: {P["muted"]}; line-height: 1.6; margin-bottom: 10px;
}}
